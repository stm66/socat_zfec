...
char RECV_DIR[MAX_PATH]      = "/home/user/work/par2/receiver/data";
char LOG_FILE[MAX_PATH]      = "/home/user/work/par2/receiver/log/receiver.log";
...
int main() {
    ...
    if (recv_metadata(meta_sock, basename, &total_parts, &required_parts) != 0) {
        log_msg("Не удалось получить метаданные");
        close(meta_sock);
        fclose(log_fp);
        return 1;
    }
    ...
    snprintf(tmp_dir, sizeof(tmp_dir), "%s/tmp_%s", RECV_DIR, basename);
    char cmd[MAX_PATH];
    ...
    if (reconstruct_file(tmp_dir, basename, k, m, RECV_DIR) != 0) {
        log_msg("Ошибка восстановления файла");
    }
